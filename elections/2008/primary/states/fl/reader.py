...
def fetchData():
	urllib.urlretrieve( private.csvFeedUrl, 'fl_text_output_for_mapping.csv' )
	
def readVotes( data, party ):
	print 'Processing vote data'
	reader = csv.reader( open( 'fl_text_output_for_mapping.csv', 'rb' ) )
	header = []
	while header == []:
		header = reader.next()
	#print header
...
def setData( header, county, row, party ):
	...
	if party != 'democrat':
		setVotes( header, county, row, 11, 'republican' )
	if party != 'republican':
