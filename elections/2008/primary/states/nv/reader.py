...
def fetchData():
	urllib.urlretrieve( private.csvFeedUrl, 'nv_text_output_for_mapping.csv' )
	
def readVotes( data, party ):
	print 'Processing vote data'
	reader = csv.reader( open( 'nv_text_output_for_mapping.csv', 'rb' ) )
	header = []
	while header == []:
		header = reader.next()
	#print header
	for row in reader:
		...
		if countyName == '*':
			if( ( party == 'democrat' and row[3] ) or ( party == 'republican' and row[12] ) ):
...
def setData( header, county, row, party ):
	...
	if party != 'democrat':
		setVotes( header, county, row, 12, 'republican' )
	if party != 'republican':
