...
*/
// Global variables for objects created in the slides.
...
function mySetSize(node, width, height) {
...
function setupSlide(index) {
  ...
  if (slideManager.title && slide.title) {
    ...
    slideManager.title.appendChild(
        document.createTextNode(egnum + ". " + slide.title));
  }
...
function setupTextSlidePreserveMap() {
  setupTextSlide();
  if (!map) {
    map = new GMap2(document.getElementById("map"));
    showWorld();
...
function showScreenshot(url, width, height) {
  ...
  mySetSize(panel, width, height);
  setZIndex(panel, 100);
  setVisible(panel, true);
  var shadow = el("shadow");
  if (shadow) {
    setLocation(shadow, x + 12, y + 12);
    mySetSize(shadow, width, height);
    setZIndex(shadow, 99);
...
// In case slides are viewed out of order, initialize the polygon
// expected for a given slide
function maybeSetupPolygon(step) {
  setupTextSlidePreserveMap();
  showSite();
  if (!polygon) {
    polygon = new GPolygon([], 
			   "#ff0000", 2, 0.7, "#ff0000", 0.2);
...
// In case slides are viewed out of order, initialize the polyline
// expected for a given slide
