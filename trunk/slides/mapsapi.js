...
var loc = {
  'paloalto': new GLatLng(37.4419, -122.1419),
  'liconf': new GLatLng(37.7843, -122.4085),
  'oak': new GLatLng(37.7156, -122.2106),
  'usydney': new GLatLng(-33.888711, 151.189492),
  'hamburg': new GLatLng(53.553108,9.972239)
};

function initmap(locName) {
  map = new GMap2($('bgmap'));
  map.setCenter(loc[locName], 13);
...
function control(which) {
  ...
  if (c) {
    ...
    var c1 = new controltypes_[which][0];
    map.addControl(c1, controltypes_[which][1]);
    controls_[which] = c1;
  }
...
function go(where) {
  map.setCenter(go_[where][0], go_[where][1]);
}
...
function behaviorMarkerOver() {
  ...
  map.addOverlay(marker);
  GEvent.addListener(marker, "mouseover", function() {
    marker.setImage("images/marker-yellow.png");
  });
