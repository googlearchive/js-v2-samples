...
PhotoLayerCallback.prototype.createMarker = function(photo, baseIcon) {
  ...
  var html = "<div id='infowin' style='height:200px;width:250px;'><br/>" +
...
function PhotoLayer(map, opt_opts) {
  ...
  GEvent.addListener(map, "moveend", function() {
    if (me.enabled) {
      ...
      var radius = bounds.getCenter().distanceFrom(bounds.getNorthEast());
me.load(me, {center: bounds.getCenter().toUrlValue(6), radius: radius});
    }
  });
}
...
PhotoLayer.prototype.load = function(panoLayer, options) {
  ...
  var url = "http://stage.gdata.youtube.com/feeds/api/videos?location=" +
     options.center + "!&location-radius=" + options.radius + "m" + 
    "&alt=json-in-script&";
